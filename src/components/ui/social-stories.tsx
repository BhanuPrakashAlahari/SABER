...
export function SocialStories({
    ...
    const goNext = useCallback(() => {
        ...
        if (currentIndex < stories.length - 1) {
            ...
            // End of stories for this profile
            if (onNextProfile) {
                onNextProfile()
            } else {
                handleClose()
                if (onAllStoriesViewed) onAllStoriesViewed()
            }
        }
    }, [currentIndex, stories.length, resetTiming, onNextProfile, onAllStoriesViewed])
    ...
    const goPrev = useCallback(() => {
        ...
        if (currentIndex > 0) {
            ...
            // At start of stories for this profile
            if (onPrevProfile) onPrevProfile()
        }
    }, [currentIndex, resetTiming, onPrevProfile])
    ...
    return (
      ...
      */}
            <div className="relative w-16 h-16 sm:w-20 sm:h-20 cursor-pointer z-10 flex-shrink-0">
                {!isOpen && (
                    <motion.div
                        layoutId={`story-trigger-${profile.name}`} // Unique ID for multiple instances
                        onClick={handleOpen}
                        className="absolute inset-0 rounded-full p-[4px]"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                    >
                        {/* Gradient Ring or Gray Ring if Visited */}
                        {isVisited ? (
                        ...
                        <div className="absolute -bottom-6 inset-x-0 text-center">
                            <span className="text-xs text-white truncate block w-full">{profile.name}</span>
                        </div>
      ...
      */}
            {createPortal(
              ...
              */}
                            <motion.div
                                layoutId={`story-card-modal-${profile.name}`}
